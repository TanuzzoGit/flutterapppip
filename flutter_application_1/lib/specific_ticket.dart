...
class _SpecificTicketState extends State<SpecificTicket> {
  ...
  Future<void> _fetchTicketDetails() async {
    final response = await http.get(
      Uri.parse('${dotenv.env['IP_ADDR']}/api/appunti/${widget.ticketId}'),
    );
    ...
    if (response.statusCode == 200) {
      setState(() {
        ...
        _selectedResponsabile = _ticketDetails!['dipendenteResponsabile'] ?? colleghi.first;
      });
    } else {
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            ...
                            DropdownButtonExample(
                              dropdownValue: _selectedResponsabile ?? colleghi.first,
                              list: colleghi,
                              onChanged: (String? newValue) {
                                if (newValue != null) {
                                  setState(() {
                                  ...
                                  print("Responsabile selezionato: $newValue");
                                  // Qui puoi aggiungere la logica per salvare nel database
                                }
                              },
                            ),
...
Row(
                          children: [
                            ...
                            Text(
                              _ticketDetails!['dataCreazione'].substring(0,10) ?? 'N/A',
                              style: TextStyle(fontSize: 16),
                            ),
                          ],
                        ),
  ...
  final url = '${dotenv.env['IP_ADDR']}/api/appunti/${widget.ticketId}';
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Errore durante il salvataggio: ${response.statusCode}')),
      );
      print(response.body);
    }
  } catch (e) {
